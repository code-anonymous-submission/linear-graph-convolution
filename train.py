...
parser.add_argument('--folds', type=int, default=10,
                    help='k-folds for cross-validation.')
...
def SGC_main(adj, features, labels, epochs, lr, dropout, weight_decay, use_relu, idx_train, idx_val, idx_test):
    
    model = get_model('SGC', features.size(1), 
                      labels.max().item()+1, args.hidden,        
                      dropout, args.cuda, use_relu)
    ...
    model, acc_val, train_time, train_acc, train_loss = train_regression(model, 
...
def k_fold_run():
    ...
    for train_index, test_index in skf.split(X, y):
        ...
        idx_val = torch.LongTensor(X_test)
...
print("Average accuracy over {} folds: {:.4f}+-{:.2f}".format(args.folds, np.mean(fold_acc_list)*100, np.std(fold_acc_list)*100))
